...
$query = "select p.proposal_number, p.category_descrip, p.title, p.abstract, s.first, s.last from proposal p, axafusers..person_short s
WHERE rank >= 3.5 and type = \"THEORY\" and p.piid=s.pers_id and p.ao_str =\"".$cyc."\"";
$query = "select p.proposal_number, p.category_descrip, p.title, p.abstract, s.first, s.last from proposal p, axafusers..person_short s
WHERE rank >= 3.5 and type = \"ARCHIVE\" and p.piid=s.pers_id and p.ao_str =\"".$cyc."\"";
...
def do_the_thing(proplist):
    ...
    <center>
    <h1>Accepted Cycle {0} Theory Proposals</h1>
    <h2>{1}</h2>
    ...
    if proplist == archive_list or proplist == theory_list:
        ...
        for proposal in current_list:
            file.write(table_builder(proposal,ATflag))
    file.write( "</table>")
